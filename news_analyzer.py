...
class NewsAnalyzer:
    ...
    def fetch_news_data(self):
        for category in self.categories:
            ...
            all_articles = self.newsapi_client.get_everything(q=category, from_param=from_date, to=to_date,
                                                              language='en')
    ...
    def preprocess_sentiment_data(self):
        ...
        self.news_with_cat['stemmed_tokens'] = self.news_with_cat['tokens']\
            .apply(lambda x: [stemmer.stem(word) for word in x])

        lemmatizer = WordNetLemmatizer()
        self.news_with_cat['lemmatized_tokens'] = self.news_with_cat['tokens']\
            .apply(lambda x: [lemmatizer.lemmatize(word) for word in x])

        sia = SentimentIntensityAnalyzer()
        self.news_with_cat['compound_sentiment'] = self.news_with_cat['title']\
